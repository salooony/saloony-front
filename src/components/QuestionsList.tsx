...
export default function QuestionsList() {
  ...
  const {
    palette: {
      primary: { main: accentColor, light: panelBackground }
    }
  } = useTheme();
  ...
  return (
    <Box>
      ...
      {faqData.map((item) => (
        ...
        >
          <AccordionSummary
            expandIcon={<CaretDownOutlined style={{ fontSize: 20, marginLeft: 8, color: accentColor }} />}
          ...
          <AccordionDetails>
            <Typography variant="body2" sx={{ color: accentColor, pl: 2, fontSize: 16 }}>
              {item.answer}
            </Typography>
          </AccordionDetails>
        </Accordion>
