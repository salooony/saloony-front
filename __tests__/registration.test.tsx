...
const renderRegistrationForm = (providers: Record<string, Provider> | null = {}) =>
  render(
...
describe('AuthRegister (Registration) - Unit Tests', () => {
  describe('Rendering', () => {
    ...
    it('starts oauth flow for Auth0 and Cognito provider buttons', async () => {
      const user = userEvent.setup();
      renderRegistrationForm({
        auth0: { id: 'auth0', name: 'Auth0' },
        cognito: { id: 'cognito', name: 'Cognito' },
      });

      await user.click(screen.getByRole('button', { name: /auth0/i }));
      await user.click(screen.getByRole('button', { name: /cognito/i }));

      expect(mockedSignIn).toHaveBeenCalledWith('auth0', { callbackUrl: APP_DEFAULT_PATH });
      expect(mockedSignIn).toHaveBeenCalledWith('cognito', { callbackUrl: APP_DEFAULT_PATH });
    });

    it('ignores login/register providers and does not render oauth provider buttons for them', () => {
      renderRegistrationForm({
        login: { id: 'login', name: 'Credentials Login' },
        register: { id: 'register', name: 'Credentials Register' },
      });

      expect(screen.queryByText(/sign up with/i)).not.toBeInTheDocument();
      expect(screen.queryByRole('button', { name: /google/i })).not.toBeInTheDocument();
      expect(screen.queryByRole('button', { name: /auth0/i })).not.toBeInTheDocument();
      expect(screen.queryByRole('button', { name: /cognito/i })).not.toBeInTheDocument();
    });

    it('renders FirebaseSocial fallback buttons when providers are not provided', async () => {
      const user = userEvent.setup();
      renderRegistrationForm(null);

      const googleButton = screen.getByRole('button', { name: /google/i });
      const auth0Button = screen.getByRole('button', { name: /auth0/i });
      const cognitoButton = screen.getByRole('button', { name: /cognito/i });

      expect(googleButton).toBeInTheDocument();
      expect(auth0Button).toBeInTheDocument();
      expect(cognitoButton).toBeInTheDocument();

      await user.click(googleButton);
      await user.click(auth0Button);
      await user.click(cognitoButton);
